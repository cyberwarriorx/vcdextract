...
private:
   ...
   int copyDirRecord(unsigned char *buffer, dirrec_struct *dirrec);
	...
	enum errorcode readDirRecords(uint32_t lba, uint32_t dirrecsize, dirrec_struct **dirrec, uint32_t *numdirrec, uint32_t parent);
	enum errorcode loadCompleteRecordSet(pvd_struct *pvd, dirrec_struct **dirrec, uint32_t *numdirrec);
	void setPathSaveTime(char *path, dirrec_struct *dirrec);
	enum errorcode extractFiles(dirrec_struct *dirrec, uint32_t numdirrec, const char *dir);
	enum errorcode createPaths(const char *dir, ptr_struct *ptr, int numptr, dirrec_struct *dirrec, uint32_t numdirrec);
	enum errorcode extractCDDA(dirrec_struct *dirrec, uint32_t numdirrec, const char *dir);
	void createDB(pvd_struct *pvd, dirrec_struct *dirrec, uint32_t numdirrec, DBClass *db);
